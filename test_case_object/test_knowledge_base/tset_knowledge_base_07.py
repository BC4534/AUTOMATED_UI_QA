...
class TestKnowledgeBase07():
    ...
    def test_knowledge_base_07_1(self,login_driver):
        ...
        try:
            ...
            status = knowledge_base_page.get_knowledge_status_by_title(problem_summary_data["knowledge_title"])
            if status != 1:
                assert status == "审核中"
    ...
    def test_knowledge_base_07_2(self, login_driver):
        ...
        try:
            ...
            knowledge_base_page.click_edit_button_by_title(rule_regulation_data["knowledge_title"])
            knowledge_base_page.click_submit_button()
            status = knowledge_base_page.get_knowledge_status_by_title(rule_regulation_data["knowledge_title"])
            if status != 1:
                assert status == "审核中"
        except Exception as e:
            logger.error(f"{self.__class__.__name__}执行用例失败,错误信息为:{e}")
    ...
    def test_knowledge_base_07_3(self, login_driver):
        ...
        try:
            logger.info(f"{self.__class__.__name__}开始执行用例")
            knowledge_base_page.switch_to_knowledge_base_page()
            knowledge_base_page.click_audit_button_by_title(rule_regulation_data["knowledge_title"])
            ...
            assert knowledge_base_page.get_knowledge_status_by_title(rule_regulation_data["knowledge_title"]) == "待修改"
        except Exception as e:
            logger.error(f"{self.__class__.__name__}执行用例失败,错误信息为:{e}")
            knowledge_base_page.get_screenshot_png(f"{self.__class__.__name__}")
            raise e
    ...
    def test_knowledge_base_07_4(self, login_driver):
        ...
        try:
            logger.info(f"{self.__class__.__name__}开始执行用例")
            knowledge_base_page.switch_to_knowledge_base_page()
            knowledge_base_page.click_audit_button_by_title(problem_summary_data["knowledge_title"])
            knowledge_base_page.click_back_button()
            knowledge_base_page.click_audit_button_by_title(problem_summary_data["knowledge_title"])
            knowledge_base_page.click_audit_page_audit_button()
            knowledge_base_page.audit_operation("审核通过", "通过原因")
            knowledge_base_page.click_confirm_delete_button()
            assert knowledge_base_page.get_knowledge_status_by_title(problem_summary_data["knowledge_title"]) == "审核成功"
        except Exception as e:
            logger.error(f"{self.__class__.__name__}执行用例失败,错误信息为:{e}")
            knowledge_base_page.get_screenshot_png(f"{self.__class__.__name__}")
            raise e
    ...
    def test_knowledge_base_07_5(self, login_driver):
        ...
        try:
            ...
            knowledge_base_page.click_edit_button_by_title(rule_regulation_data["knowledge_title"])
            knowledge_base_page.click_submit_button()
            assert knowledge_base_page.get_knowledge_status_by_title(rule_regulation_data["knowledge_title"]) == "审核中"
        except Exception as e:
            logger.error(f"{self.__class__.__name__}执行用例失败,错误信息为:{e}")
            knowledge_base_page.get_screenshot_png(f"{self.__class__.__name__}")
            raise e
