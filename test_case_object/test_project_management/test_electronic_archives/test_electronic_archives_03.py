...
class TestElectronicArchives03:
    ...
    def test_electronic_archives_03_1(self, login_driver):
        ...
        try:
            ...
            assert (
                add_election_archives_page.get_init_time_required_text()
                == "请选择立项时间"
            )
            assert (
                add_election_archives_page.get_project_name_required_text()
                == "请输入项目名称"
            )
            assert (
                add_election_archives_page.get_project_install_power_required_text()
                == "请输入项目装机功率"
            )
            assert (
                add_election_archives_page.get_project_install_capacity_required_text()
                == "请输入项目装机容量"
            )
            assert (
                add_election_archives_page.get_project_stage_required_text()
                == "请选择项目阶段"
            )
            assert (
                add_election_archives_page.get_project_progress_required_text()
                == "请选择项目进度"
            )
            assert (
                add_election_archives_page.get_project_type_required_text()
                == "请选择项目类型"
            )
            assert (
                add_election_archives_page.get_product_type_required_text()
                == "请选择产品类型"
            )
            assert (
                add_election_archives_page.get_project_area_required_text()
                == "请选择所属区域"
            )
            assert (
                add_election_archives_page.get_is_support_inspection_standard_required_text()
                == "请选择是否支持标准巡检"
            )
    ...
    def test_electronic_archives_03_2(self, login_driver):
        ...
        try:
            ...
            add_election_archives_page.click_save_button()
            assert (
                add_election_archives_page.get_owner_required_text() == "请输入业主名称"
            )
            assert (
                add_election_archives_page.get_battery_box_number_required_text()
                == "请输入电池仓数量"
            )
            assert (
                add_election_archives_page.get_bms_vendor_required_text()
                == "请输入BMS厂商"
            )
        except Exception as e:
            logger.error(f"{self.__class__.__name__} 测试用例执行失败，错误信息为：{e}")
    ...
    def test_electronic_archives_03_3(self, login_driver):
        ...
        try:
            ...
            add_election_archives_page.click_save_button()
            assert (
                add_election_archives_page.get_maintain_implement_time_required_text()
                == "请输入实施计划时间"
            )
            assert (
                add_election_archives_page.get_maintain_implement_person_required_text()
                == "请选择实施负责人"
            )
    ...
    def test_electronic_archives_03_4(self, login_driver):
        ...
        try:
            ...
            add_election_archives_page.click_save_button()
            assert (
                add_election_archives_page.get_operation_maintenance_person_required_text()
                == "请选择运维负责人"
            )
            assert (
                add_election_archives_page.get_first_inspection_time_required_text()
                == "请选择首次巡检时间"
            )
            assert (
                add_election_archives_page.get_inspection_cycle_required_text()
                == "请选择巡检周期"
            )
            add_election_archives_page.click_close_button()
