...
class TestElectronicArchives03:
    """
    新增项目必填项效验
    """
    @allure.feature("项目管理模块")
    @allure.story("电子档案功能")
    @allure.title("新增项目-基础资料维护必填项")
    @allure.description("新增项目-验证基础资料维护必填项")
    def test_electronic_archives_03_1(self, login_driver):
        ...
        try:
            ...
            add_election_archives_page.click_save_button()
            assert add_election_archives_page.get_init_time_required_text() == "请选择立项时间"
            assert add_election_archives_page.get_project_name_required_text() == "请输入项目名称"
            assert add_election_archives_page.get_project_install_power_required_text() == "请输入项目装机功率"
            assert add_election_archives_page.get_project_install_capacity_required_text() == "请输入项目装机容量"
            assert add_election_archives_page.get_project_stage_required_text() == "请选择项目阶段"
            assert add_election_archives_page.get_project_progress_required_text() == "请选择项目进度"
            assert add_election_archives_page.get_project_type_required_text() == "请选择项目类型"
            assert add_election_archives_page.get_product_type_required_text() == "请选择产品类型"
            assert add_election_archives_page.get_project_area_required_text() == "请选择所属区域"
            assert add_election_archives_page.get_is_support_inspection_standard_required_text() == "请选择是否支持标准巡检"
    ...
    @allure.description("新增项目-验证项目详细资料维护必填项")
    def test_electronic_archives_03_2(self, login_driver):
        add_election_archives_page = AddOperationMaintenanceManagementInfoPage(login_driver)
        try:
            logger.info(f"{self.__class__.__name__} 开始执行用例")
            add_election_archives_page.fill_basic_information(init_time=base_info_data["init_time"],
            ...
            add_election_archives_page.click_save_button()
            assert add_election_archives_page.get_owner_required_text() == "请输入业主名称"
            assert add_election_archives_page.get_battery_box_number_required_text() == "请输入电池仓数量"
            assert add_election_archives_page.get_bms_vendor_required_text() == "请输入BMS厂商"
        except Exception as e:
            logger.error(f"{self.__class__.__name__} 测试用例执行失败，错误信息为：{e}")
            add_election_archives_page.get_screenshot_png(f"{self.__class__.__name__}")
            raise e
    @allure.description("新增项目-维护实施信息必填项")
    def test_electronic_archives_03_3(self, login_driver):
        add_election_archives_page = AddOperationMaintenanceManagementInfoPage(login_driver)
        try:
            logger.info(f"{self.__class__.__name__} 开始执行用例")
            ...
            add_election_archives_page.click_next_button()
            add_election_archives_page.click_save_button()
            assert add_election_archives_page.get_maintain_implement_time_required_text() == "请输入实施计划时间"
            assert add_election_archives_page.get_maintain_implement_person_required_text() == "请选择实施负责人"
    ...
    @allure.description("新增项目-运维管理信息必填项")
    def test_electronic_archives_03_4(self, login_driver):
        add_election_archives_page = AddOperationMaintenanceManagementInfoPage(login_driver)
        try:
            logger.info(f"{self.__class__.__name__} 开始执行用例")
            ...
            add_election_archives_page.click_next_button()
            add_election_archives_page.click_save_button()
            assert add_election_archives_page.get_operation_maintenance_person_required_text() == "请选择运维负责人"
            assert add_election_archives_page.get_first_inspection_time_required_text() == "请选择首次巡检时间"
            assert add_election_archives_page.get_inspection_cycle_required_text() == "请选择巡检周期"
            add_election_archives_page.click_close_button()
