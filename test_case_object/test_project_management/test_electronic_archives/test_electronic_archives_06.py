...
@allure.story("电子档案功能")
@allure.title("编辑项目")
class TestElectronicArchives06:

    @allure.description("编辑项目,修改名称")
    def test_electronic_archives_06_1(self, login_driver):
        ...
        try:
            ...
            new_project_name = "UI自动化测试项目修改"
            add_election_archives_page._click_first_project_edit_button()
            add_election_archives_page.input_project_name(new_project_name)
            add_election_archives_page.click_save_button()
            add_election_archives_page.click_close_button()
            assert add_election_archives_page.get_first_project_name() == new_project_name
            add_election_archives_page._click_first_project_edit_button()
            add_election_archives_page.input_project_name(first_project_name)
            add_election_archives_page.click_save_button()
            add_election_archives_page.click_close_button()
            assert add_election_archives_page.get_first_project_name() == first_project_name
            ...
            if add_election_archives_page.get_first_project_name() != first_project_name:
                add_election_archives_page.input_project_name(first_project_name)
                add_election_archives_page.click_save_button()
                add_election_archives_page.click_close_button()
            raise e

    @allure.description("验证编辑项目弹框文本")
    def test_electronic_archives_06_2(self, login_driver):
        add_election_archives_page = AddOperationMaintenanceManagementInfoPage(login_driver)
        try:
            logger.info(f"{self.__class__.__name__} 开始执行用例")
            add_election_archives_page.switch_to_electronic_archives()
            add_election_archives_page._click_first_project_edit_button()
            assert add_election_archives_page.get_add_edit_project_title() == "编辑项目"
            add_election_archives_page.click_close_button()
        except Exception as e:
            logger.error(f"{self.__class__.__name__} 测试用例执行失败，错误信息为：{e}")
            add_election_archives_page.get_screenshot_png(f"{self.__class__.__name__}")
