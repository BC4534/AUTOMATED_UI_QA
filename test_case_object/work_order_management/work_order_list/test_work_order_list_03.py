...
class TestWorkOrderList03(object):
    ...
    def test_work_order_list_03(self, login_driver):
        ...
        try:
            ...
            assert (
                work_order_list_page.get_work_order_type_required_text()
                == "请选择工单类型"
            )
            assert (
                work_order_list_page.get_work_order_name_required_text()
                == "请输入工单名称"
            )
            assert (
                work_order_list_page.get_work_order_description_required_text()
                == "请输入工单描述"
            )
            assert (
                work_order_list_page.get_association_project_required_text()
                == "请选择关联项目"
            )
            assert (
                work_order_list_page.get_responsible_person_required_text()
                == "请选择负责人"
            )
            assert (
                work_order_list_page.get_plan_start_time_required_text()
                == "请选择计划开始时间"
            )
            assert (
                work_order_list_page.get_plan_end_time_required_text()
                == "请选择计划结束时间"
            )
