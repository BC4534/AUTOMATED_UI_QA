...
class KnowledgeBasePage(BasePage):
    ...
    # ------新增知识页面操作----
    @allure.step("新增知识")
    def fill_add_knowledge_data(self, knowledge_type="", knowledge_title="", associated_project="",
                                associated_device_type="", abnormal="", knowledge_content=""):
        self.click_element(KnowledgeBaseLocator.knowledge_type_select_loc)
        if knowledge_type == "问题总结":
            self._fill_add_knowledge_data_type_problem_summary(knowledge_title, associated_project,
                                                               associated_device_type, abnormal, knowledge_content)
        elif knowledge_type == "规章制度":
            self._fill_add_knowledge_data_type_rule_regulation(knowledge_title, knowledge_content)
        elif knowledge_type == "行业标准":
            self._fill_add_knowledge_data_type_industry_standard(knowledge_title, knowledge_content)
        elif knowledge_type == "项目资料":
            self._fill_add_knowledge_data_type_project_info(knowledge_title, associated_project, knowledge_content)
        else:
    ...
    def __fill_add_knowledge_content(self, knowledge_content):
        ...
        self.send_keys_by_clear_and_typing(KnowledgeBaseLocator.content_input_loc, knowledge_content)

    # 填写新增知识数据 问题总结类
    def _fill_add_knowledge_data_type_problem_summary(self, knowledge_title, associated_project, associated_device_type,
                                                      abnormal, knowledge_content):
        self.click_element(KnowledgeBaseLocator.problem_summary_option_loc)
        self.random_sleep(0.5)
        self.send_keys_by_clear_and_typing(KnowledgeBaseLocator.title_input_loc, knowledge_title)
        self.random_sleep(0.5)
        ...
        try:
            ...
            logger.warning(f"没有找到设备类型{associated_device_type},选择默认第一个设备类型")
    ...
    # 填写新增知识数据 规章制度类
    def _fill_add_knowledge_data_type_rule_regulation(self, knowledge_title, knowledge_content):
    ...
    # 填写新增知识数据 行业标准类
    def _fill_add_knowledge_data_type_industry_standard(self, knowledge_title, knowledge_content):
    ...
    # # 填写新增知识数据 项目资料类
    def _fill_add_knowledge_data_type_project_info(self, knowledge_title, associated_project, knowledge_content):
    ...
    def input_knowledge_name_query_condition(self,knowledge_name):
        self.send_keys_by_clear_and_typing(KnowledgeBaseLocator.knowledge_name_input_loc,knowledge_name)
        self.random_sleep(0.5)
    ...
    def input_writer_query_condition(self, writer):
        self.send_keys_by_clear_and_typing(KnowledgeBaseLocator.writer_select_loc, writer)
    ...
    def get_first_no_abnormal_link(self):
        try:
            for i in range(2,12):  # //*[@class="ant-table-tbody"]/tr[2]/td[9]
                if self.text((By.XPATH, f"//*[@class='ant-table-tbody']/tr[{i}]/td[9]")) != "":
                    return self.text((By.XPATH, f"//*[@class='ant-table-tbody']/tr[{i}]/td[9]"))
        except:
    ...
    def edit_operation(self,new_knowledge_title,new_knowledge_content):
        ...
        self.send_keys_by_clear_and_typing(KnowledgeBaseLocator.title_input_loc, new_knowledge_title)
    ...
    def get_knowledge_status_by_title(self,knowledge_title):
        try:
            for i in range(2,12):
                if self.text((By.XPATH, f"//*[@class='ant-table-tbody']/tr[{i}]/td[3]")) == knowledge_title:
                    return self.text((By.XPATH, f"//*[@class='ant-table-tbody']/tr[{i}]/td[2]"))
        except:
            logger.warning("没有找到第一条知识状态")
            return 1
        ...
        for i in range(2,12):
            try:
                if self.text((By.XPATH, f"//*[@class='ant-table-tbody']/tr[{i}]/td[3]")) == knowledge_title:
                    ...
                    self.click_element((By.XPATH, f'//*[@class="ant-table-tbody"]/tr[{i}]/td[last()]//button/span[text()="编辑"]/..'))
                    self.random_sleep(0.5)
                    break
            except:
                logger.warning("没有找到第一条知识状态")
    ...
    def audit_operation(self,audit_option,audit_remark):
        ...
        self.send_keys_by_clear_and_typing(KnowledgeBaseLocator.audit_remark_input_loc, audit_remark)
