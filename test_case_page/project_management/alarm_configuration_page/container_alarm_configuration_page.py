...
class ContainerAlarmConfigurationPage(BasePage):
    ...
    def click_batch_update_cancel_button(self):
        self.click_element(ContainerAlarmConfigurationLocator.batch_update_cancel_button_loc)
        self.random_sleep(0.5)

    # 点击批量更新关闭按钮
    def click_batch_update_close_button(self):
        self.click_element(ContainerAlarmConfigurationLocator.batch_update_close_button_loc)
        self.random_sleep(0.5)

    # 点击批量更新确定按钮
    def click_batch_update_confirm_button(self):
        self.click_element(ContainerAlarmConfigurationLocator.batch_update_confirm_button_loc)
        self.random_sleep(0.5)

    # 翻页功能
    def test_page_turning(self):
        if self.visibility_of_element_located(ContainerAlarmConfigurationLocator.second_page_loc):
            ...
            a = self.get_attribute(ContainerAlarmConfigurationLocator.first_page_loc, "class")
    ...
    def check_batch_update_close(self):
        try:
            self.visibility_of_element_located(ContainerAlarmConfigurationLocator.batch_update_file_format_loc)
            return False
        except:
            return True
