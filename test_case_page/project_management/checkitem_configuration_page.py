...
class CheckItemConfigurationPage(BasePage):
    ...
    def click_delete_checkitem_confirm_button(self):
        self.click_element(
            CheckitemConfigurationLocator.delete_checkitem_button_confirm_loc
        )
        time.sleep(1)
    ...
    def click_delete_checkitem_cancel_button(self):
        self.click_element(
            CheckitemConfigurationLocator.delete_checkitem_button_cancel_loc
        )
        time.sleep(1)
    ...
    def click_first_checkitem_edit_button(self):
        self.click_element(
            CheckitemConfigurationLocator.first_checkitem_edit_button_loc
        )
        time.sleep(0.5)
    ...
    def click_add_checkitem_confirm_button(self):
        self.click_element(
            CheckitemConfigurationLocator.add_checkitem_button_confirm_loc
        )
        time.sleep(0.5)
    ...
    def click_add_checkitem_cancel_button(self):
        self.click_element(
            CheckitemConfigurationLocator.add_checkitem_button_cancel_loc
        )
        time.sleep(0.5)
    ...
    def get_add_checkitem_content_required_text(self):
        return self.text(
            CheckitemConfigurationLocator.add_checkitem_content_required_loc
        )
    ...
    def get_add_checkitem_remark_required_text(self):
        return self.text(
            CheckitemConfigurationLocator.add_checkitem_remark_required_loc
        )
    ...
    def add_checkitem(
        ...
        self.send_keys_by_clear_and_typing(
            CheckitemConfigurationLocator.add_checkitem_name_input_loc, checkitem_name
        )
        self.click_element(CheckitemConfigurationLocator.add_checkitem_type_select_loc)
        self.random_sleep(0.5)
        if checkitem_type == "空调系统":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_option_air_loc
            )
        elif checkitem_type == "电池室或电池仓":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_option_battery_loc
            )
        elif checkitem_type == "储能电池及电池管理系统（BMS）":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_option_bms_loc
            )
        elif checkitem_type == "EMS系统":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_option_ems_loc
            )
        elif checkitem_type == "消防系统":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_option_fire_loc
            )
        elif checkitem_type == "汇流柜":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_option_hui_loc
            )
        elif checkitem_type == "液冷系统":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_option_liquid_loc
            )
        elif checkitem_type == "储能变流器（PCS）":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_option_pcs_loc
            )
        else:
        ...
        self.random_sleep(0.5)
        self.send_keys_by_clear_and_typing(
            CheckitemConfigurationLocator.add_checkitem_content_input_loc,
            checkitem_content,
        )
        self.random_sleep(0.5)
        if checkitem_photo == "是":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_photo_yes_loc
            )
        ...
        if checkitem_remark == "是":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_remark_yes_loc
            )
        elif checkitem_remark == "否":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_remark_no_loc
            )
        else:
    ...
    def edit_checkitem(
        ...
        if checkitem_name != "":
            self.send_keys_by_clear_and_typing(
                CheckitemConfigurationLocator.add_checkitem_name_input_loc,
                checkitem_name,
            )
        if checkitem_type != "":
            self.click_element(
                CheckitemConfigurationLocator.add_checkitem_type_select_loc
            )
            self.random_sleep(0.5)
            if checkitem_type == "空调系统":
                self.click_element(
        ...
        if checkitem_photo != "":
            if checkitem_photo == "是":
                ...
                self.click_element(
                    CheckitemConfigurationLocator.add_checkitem_photo_no_loc
                )
            else:
                self.logger.error("新增巡检项拍照是否输入错误,默认选择否")
                self.click_element(
    ...
    def get_last_checkitem_name(self):
        ...
        time.sleep(1)
        return self.text(
            CheckitemConfigurationLocator.current_page_last_checkitem_name_loc
        )
    ...
    def get_first_checkitem_name(self):
        time.sleep(1)
        return self.text(
            CheckitemConfigurationLocator.current_page_first_checkitem_name_loc
        )
    ...
    def get_checkitem_name_list(self):
        ...
        while True:
            for i in range(2, 12):
                try:
                    checkitem_name_list.append(
                        self.text(
                            (
                                By.XPATH,
                                f'//*[@class="ant-table-tbody"]/tr[{i}]/td[2]/div',
                            )
                        )
                    )
                ...
                try:
                    if (
                        ...
                        self.click_element(
                            (
                                By.XPATH,
                                f'//*[@class="ant-table-tbody"]/tr[{i}]/td//span',
                            )
                        )
                except:
                    break
    ...
    def is_next_page_enable(self):
        return self.get_attribute(
            CheckitemConfigurationLocator.next_page_loc, "aria-disabled"
        )
    ...
    def search_checkitem_by_name(self, checkitem_name):
        self.send_keys_by_clear_and_typing(
            CheckitemConfigurationLocator.checkitem_name_input_loc, checkitem_name
        )
        time.sleep(1)
