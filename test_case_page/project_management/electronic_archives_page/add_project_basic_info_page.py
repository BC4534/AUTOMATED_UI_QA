...
class AddProjectBaseInfoPage(ElectronicArchivesPage):
    ...
    # 新增项目 填写项目基础资料信息
    def fill_basic_information(
        self,
        init_time: str,
        project_name,
        project_install_power,
        project_install_capacity,
        project_stage,
        project_progress,
        project_type,
        product_type,
        outdoor_cabinet_type,
        project_area,
        is_support_inspection,
    ):
        ...
        self.input_project_install_capacity(
            project_install_capacity
        )  # 输入项目装机容量
        ...
        self.is_support_inspection_standard(
            is_support_inspection
        )  # 选择是否支持巡检标准
    ...
    def input_project_name(self, project_name):
        ...
        self.send_keys_by_clear(
            ProjectBaseInfoLocator.project_name_input_loc, project_name
        )
        self.random_sleep(0.5)
    ...
    def input_project_install_power(self, project_install_power):
        # 输入项目装机功率
        self.send_keys_by_clear(
            ProjectBaseInfoLocator.project_install_power_input_loc,
            project_install_power,
        )
        self.random_sleep(0.5)
    ...
    def input_project_install_capacity(self, project_install_capacity):
        # 输入项目装机容量
        self.send_keys_by_clear(
            ProjectBaseInfoLocator.project_install_capacity_input_loc,
            project_install_capacity,
        )
        self.random_sleep(0.5)
        ...
        if project_stage != " ":
            if project_stage == "待实施阶段":
                self.driver.execute_script(
                    "document.getElementById('basic_phase').click();"
                )
                self.click_element(
                    ProjectBaseInfoLocator.project_stage_wait_for_implementation_loc
                )
                logger.debug("选择项目阶段为：待实施阶段")
            elif project_stage == "实施阶段":
                self.click_element(
                    ProjectBaseInfoLocator.project_stage_implementation_loc
                )
    ...
    def select_project_progress(self, project_progress):
        ...
        if project_progress != " ":
            if project_progress == "计划期":
                ...
                self.click_element(
                    ProjectBaseInfoLocator.project_progress_preparation_loc
                )
            elif project_progress == "发货期":
                self.click_element(
                    ProjectBaseInfoLocator.project_progress_send_out_goods_loc
                )
                ...
                self.click_element(
                    ProjectBaseInfoLocator.project_progress_warranty_expired_loc
                )
            else:
    ...
    def select_project_type(self, project_type):
        ...
        if project_type != " ":
            if project_type == "工商业":
                ...
                self.click_element(
                    ProjectBaseInfoLocator.project_type_electricity_side_loc
                )
            else:
    ...
    def select_product_type(self, product_type):
        ...
        if product_type != " ":
            if product_type == "户外柜":
                self.click_element(
                    ProjectBaseInfoLocator.product_type_outdoor_cabinet_loc
                )
    ...
    def select_outdoor_cabinet_type(self, outdoor_cabinet_type):
        if self.get_product_type_text() == "户外柜":
            ...
            if (
                outdoor_cabinet_type != " "
            ):  # 215户外柜国标1.0 215户外柜国标2.0 232户外柜
                if outdoor_cabinet_type == "215户外柜国标1.0":
                    self.click_element(
                        ProjectBaseInfoLocator.outdoor_cabinet_type_215_gb1_0_loc
                    )
                elif outdoor_cabinet_type == "215户外柜国标2.0":
                    self.click_element(
                        ProjectBaseInfoLocator.outdoor_cabinet_type_215_gb2_0_loc
                    )
                elif outdoor_cabinet_type == "232户外柜":
                    self.click_element(
                        ProjectBaseInfoLocator.outdoor_cabinet_type_232_loc
                    )
                else:
                    logger.error(
                        "输入 outdoor_cabinet_type不正确，默认选择215户外柜国标1.0"
                    )
                    self.click_element(
                        ProjectBaseInfoLocator.outdoor_cabinet_type_215_gb1_0_loc
                    )
                ...
                logger.info(
                    "产品类型为户外柜，outdoor_cabinet_type未填，默认选择215户外柜国标1.0"
                )
                self.click_element(
                    ProjectBaseInfoLocator.outdoor_cabinet_type_215_gb1_0_loc
                )
    ...
    def is_support_inspection_standard(self, is_support_inspection):
        ...
        if is_support_inspection != " ":
            if is_support_inspection == "是":
                self.click_element(
                    ProjectBaseInfoLocator.is_support_inspection_standard_yes_loc
                )
            elif is_support_inspection == "否":
                self.click_element(
                    ProjectBaseInfoLocator.is_support_inspection_standard_no_loc
                )
            else:
                # 巡检标准 默认选择是
    ...
    def get_project_install_capacity_required_text(self):
        return self.text(
            ProjectBaseInfoLocator.project_install_capacity_required_text_loc
        )
    ...
    def get_is_support_inspection_standard_required_text(self):
        return self.text(
            ProjectBaseInfoLocator.is_support_inspection_standard_required_text_loc
        )
